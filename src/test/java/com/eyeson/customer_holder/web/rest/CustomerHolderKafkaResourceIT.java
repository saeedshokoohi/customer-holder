...
import com.eyeson.customer_holder.config.KafkaProperties;
...
class CustomerHolderKafkaResourceIT {
    ...
    void setup() {
        ...
        CustomerHolderKafkaResource kafkaResource = new CustomerHolderKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/customer-holder-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/customer-holder-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
